...
var red1 = createSprite(200,200,30,30);
var blue1 = createSprite(200,200,30,30);
...
function ghostMove(){
  ...
  if(pacman.x-red1.x>0)signal*=1;else signal*=-1;
  red1.velocityX=signal*ghostVel+random(-change,change);
  if(pacman.y-red1.y>0)signal*=1;else signal*=-1;
  red1.velocityY=signal*ghostVel+random(-change,change); 
  ...
  if(pacman.x-blue1.x>0)signal*=1;else signal*=-1;
  blue1.velocityX=signal*ghostVel+random(-change,change);
  if(pacman.y-blue1.y>0)signal*=1;else signal*=-1;
  blue1.velocityY=signal*ghostVel+random(-change,change); 
...
function draw() {
  if (power == 0){
    ...
    if (GC > 3){
    blue1.setAnimation("blue ghost");
    blue1.scale = 0.027;
    }
  }else{
    if (GC > 0){
    red.setAnimation("good ghost");
    red.scale = 0.027;
    }
    if (GC > 2){
    red1.setAnimation("good ghost");
    red1.scale = 0.027;
    }
    if (GC > 1){
    blue.setAnimation("good ghost");
    blue.scale = 0.027;
    }
    if (GC > 3){
    blue1.setAnimation("good ghost");
    blue1.scale = 0.027;
    }
  }
  if (GameState == 0){
    ...
    if (music == 0){
      text("Press S to Unmute Song", 315, 300);
      if (keyWentDown("S")){
        music = 1;
      }
    }else{
      text("Press S to mute Song", 300, 300);
      if (keyWentDown("S")){
        music = 0;
      }
    }
  ...
  if(GameState == 1){
    if (music == 1){
    playSound("TheFatRat_Envelope_DOTA_2_Music_Pack_F1AgdpwdGkk_140.mp3", true);
    music = 0;
    }
    if (GC == 1){
    blue.destroy();
  ...
  for (var j = 0; j < walls.length; j++) {
    ...
    red1.bounceOff(walls[j]);
    blue1.bounceOff(walls[j]);
  ...
  if(pacman.isTouching(red1) && power == 0){
    lives -= 1 ;
    pacman.visible = false ;
    pacman.x = 200 ;
    pacman.y = 375 ;
    ...
    blue1.velocityY = -4 ;
    blue1.velocityX = 0 ;
  }else if(pacman.isTouching(red1) && power == 1){
    red1.x = 200;
    red1.y = 200;
  }
  ...
  if(pacman.isTouching(blue1) && power == 0){
    lives -= 1 ;
    pacman.visible = false ;
    pacman.x = 200 ;
    pacman.y = 375 ;
    ...
    blue1.velocityY = -4 ;
    blue1.velocityX = 0 ;
  }else if(pacman.isTouching(blue1) && power == 1){
    blue1.x = 200;
    blue1.y = 200;
  }
 if(keyWentDown("up")){
  ...
  text("Time: "+sec,247,140);
  text("Score: "+score,249,170);
  text("Win Score: "+winLevels[level-1],268,200);
  drawSprites();
}
  if(GameState == 2){
    background("black");
    ...
    fill("White");
    text("Press Y to Go to Next Page", 325, 345);
    ...
    text("(4)", 210, 290);
    fill("White");
    text("Press Y to Go to Previous Page", 350, 345);
    text("Press X to Back to Main Menu", 340, 380);
    if (keyWentDown("X")) {
      GameState = 0;
    }
